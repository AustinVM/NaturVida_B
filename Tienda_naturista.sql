...
CREATE PROC SP_MOSTRARPROD
AS
SELECT * FROM Productos 
--DROP PROC SP_MOSTRARPROD

CREATE PROC SP_MOSTRARPRODESPEC
@Cod INT
AS
...
CREATE PROC Mostrar_cliente_esp
@Documento int
as
...
@Docu INT
AS
DELETE  Cliente WHERE Documento = @Docu

create proc BuscarClientes
@Doc int,
@Correo varchar (100)
as 
select * from Cliente where Documento=@Doc or Correo=@Correo

create proc BuscarCliente_actualizar
@Doc int,
@Correo varchar (100)
as
select * from Cliente where Documento!=@Doc and Correo=@Correo
...
--Tabla Factura y Factura detalle
	...
	@ValUnidad INT
	AS
	INSERT INTO Factura_Detalle VALUES((select max(IdFactu) from Factura),@CodProd,@Cant,@ValUnidad)
...
@Prod INT
AS
SELECT P.Descripción,P.Cantidad,SUM(F.Cantidad) AS 'Vendido' FROM Factura_Detalle AS F --Se usa el full outer join mientras no tenga registros la tabla factura detalle luego que se llenen datos se puede usar el join
full outer JOIN Productos AS P on F.Codigo_Productos = P.Codigo
where P.Codigo = 2341 GROUP BY P.Descripción,P.Cantidad
...
create proc Mostrar_vendedores
as
select* from vendedores

create proc MostrarvendeEspe
@Codigo int
as
...
--select Usuario, Contraseña from Vendedores where Usuario='admin'
...
/****** Object:  StoredProcedure [dbo].[Codigo_existe]    Script Date: 4/24/2023 5:26:39 PM ******/
SET ANSI_NULLS ON
GO
